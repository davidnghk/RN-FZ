...
export const thingsReducer = (
  state: ThingsState = initialState,
  action: ThingsActions,
) => {
  switch (action.type) {
    case '@@things/LOAD_THINGS':
      const sortedThingsByName = action.things.sort((a, b) =>
        a.name! > b.name! ? 1 : -1,
      );
      const sortedThingsByWarningFlag = sortedThingsByName.sort(function (
        a,
        b,
      ) {
        //@ts-ignore
