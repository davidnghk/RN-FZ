...
export function loadIsUserReceiveEmail(isUserReceiveEmail: boolean, userEmailId: number | null) {
    return {
...
export type UserActions = ReturnType<typeof loadUserInfo> | ReturnType<typeof hasUnread> |
    ReturnType<typeof clearUnread> | ReturnType<typeof clearUserInfo> |
    ReturnType<typeof loadUserFontSetting> | ReturnType<typeof profileUpdateIsLoading> |
    ReturnType<typeof profilePicIsSaving> | ReturnType<typeof loadEmergencyContacts> |
    ReturnType<typeof contactIsLoading> | ReturnType<typeof loadIsUserReceiveEmail> |
...
export function updateUserNotificationSetting(userId: number, fcmToken: string, platform: string) {
    return async (dispatch: ThunkDispatch, getState: () => RootState) => {
        try {

            const token = await AsyncStorage.getItem('authToken');

            const res = await fetch(`${Config.api_server}/users/${userId}/api_update?notification_token=${fcmToken}&&notification_platform=${platform}`, {
                method: 'PUT',
                headers: {
                    'Authorization': `Bearer ${token}`
                },
...
export function resetUnread() {
    return async (dispatch: ThunkDispatch, getState: () => RootState) => {
        try {
            ...
            const res = await fetch(`${Config.api_server}/users/${user.id}/api_update?unread=0&&last_read_datetime=${today.toISOString()}`, {
                method: 'PUT',
                headers: {
                    'Authorization': `Bearer ${token}`
                },
...
export function updateProfile(userId: number, name: string, phone: string) {
    return async (dispatch: ThunkDispatch, getState: () => RootState) => {
        try {
            ...
            const res = await fetch(`${Config.api_server}/users/${userId}/api_update`, {
                method: 'PUT',
                headers: {
                    'Authorization': `Bearer ${token}`
                },
    ...
    return async (dispatch: ThunkDispatch, getState: () => RootState) => {
        try {
            ...
            if (response.status !== 'ok') {
                dispatch(actionTriggerShowModal(true, 'Update password', 'Update password fail'))
                return;
            };

            if (response.status === 'ok') {
                dispatch(actionTriggerShowModal(true, 'Update password', 'Update password success'))
...
export function getEmergencyContacts() {
    return async (dispatch: ThunkDispatch, getState: () => RootState) => {
        try {
            ...
            const userEmail = contacts.filter(contact => contact.email === user.email);
            ...
            const filterContacts = contacts.filter((contact) => contact.email != user.email);
    ...
    return async (dispatch: ThunkDispatch, getState: () => RootState) => {
        try {
            ...
            if (response.Message === 'The Stakeholderss was deleted') {
                dispatch(actionTriggerShowModal(true, 'Remove Contact', `${response.Message}`));
...
export function updateEmergencyContact(contactId: number, name: string, phone: string, email: string) {
    return async (dispatch: ThunkDispatch, getState: () => RootState) => {
        try {
            ...
            const res = await fetch(`${Config.api_server}/stakeholders/${contactId}`, {
                method: 'PUT',
                headers: {
                    'Authorization': `Bearer ${token}`
                },
...
export function createEmergencyContact(name: string, phone: string, email: string) {
    return async (dispatch: ThunkDispatch, getState: () => RootState) => {
        try {
...
export function setUserReceiveEmail(userReceiveEmail: boolean, userEmailId: number | null) {
    return async (dispatch: ThunkDispatch, getState: () => RootState) => {
        try {
            ...
            if (response.Message === 'The Stakeholderss was created') {
                dispatch(actionTriggerShowModal(true, 'Set user receive email', `${response.Message}`));
                dispatch(getEmergencyContacts());
            };
