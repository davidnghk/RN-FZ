...
const HomeScreen = (props: any) => {
  ...
  const numOfThingsOffline = things?.filter(
    thing => thing.onoff_status.toLowerCase() === 'offline',
  ).length;

  const numOfThingsWithNormal = (things?.filter(
    thing => thing.onoff_status?.toLowerCase() === 'normal',
  )).length;
  const numOfThingsWithAlarms = (things?.filter(
    thing => thing.onoff_status?.toLowerCase() === 'alarm',
  )).length;

  // Data used to make the animate prop work
  // const defaultMasterRingData = [{ x: 'Drill', y: 0 }, { x: 'Alarm', y: 0 }, { x: 'Quiet', y: 0 }, { x: 'Offline', y: 100 }]
  const defaultMasterRingData = [
    {x: 'Normal', y: 0},
    {x: 'Alarm', y: 0},
  ];
  ...
  const MAP_WIDTH = Dimensions.get('window').width * 0.83;
  ...
  useEffect(() => {
    ...
    const masterRingCheckList = [
      numOfThingsWithNormal,
      numOfThingsWithAlarms,
      numOfThingsOffline,
    ];
    ...
    const masterRingTitleList = [
      t('common:normal'),
      t('common:alarm'),
      t('common:offline'),
    ];
    const masterRingColorList = [
      COLOR.normalChartColor,
      COLOR.alarmChartColor,
      COLOR.offlineChartColor,
    ];

    for (let i = 0; i < masterRingCheckList.length; i++) {
      if (masterRingCheckList[i] > 0) {
        masterRingList.push({
          x: masterRingTitleList[i],
          y: masterRingCheckList[i],
          dataColor: masterRingColorList[i],
        });
      }
    }
      ...
      if (authorize.enabled) {
        ...
        fcmManager.getFcmToken(user.id!).then(response => {
          if (response.msg == 'success') {
            ...
            dispatch(
              UserActions.updateUserNotificationSetting(
                user.id!,
                fcmToken!,
                platform,
              ),
  ...
  const RecentAlertItem = (props: any) => {
    return (
      <TouchableOpacity
        ...
        onPress={() => {
          navigation.navigate('AlertDetailsScreen', {id: props.id});
        }}>
        <Image
          style={{height: 25, width: 25, tintColor: 'red'}}
          source={require('../../assets/images/components/siren-removebg.png')}
        />
        ...
        <View style={{width: '22%'}}>
          <CustomText
            ...
            ellipsizeMode="tail"
            numberOfLines={1}>
            {getTranslateType(props.alertType)}
          </CustomText>
          <CustomText
            style={{width: '100%'}}
            ellipsizeMode="tail"
      ...
      <TouchableOpacity
        style={[
          {...styles.deviceBlock},
          {
            backgroundColor: fireSensorGetColorFromStatusOrWarning(
              itemData.item.state,
              itemData.item.onoff_status,
              itemData.item.warning_flag,
            ),
          },
        ]}
        onPress={() => {
        ...
        <View style={styles.iconImageContainer}>
          <Image
            ...
            source={{uri: itemData.item.icon_url}}
          />
        </View>
        <View style={{width: '100%', alignItems: 'center'}}>
          <CustomText style={{}} numberOfLines={1}>
            {itemData.item.name}
          </CustomText>
        </View>
      </TouchableOpacity>
    ...
    return (
      <TouchableOpacity
        ...
        <View style={styles.iconImageContainer}>
          <Image
            style={{width: '100%', height: '100%'}}
            source={require('../../assets/images/company_logo/locations.jpeg')}
          />
        </View>
        <View style={{width: '100%', alignItems: 'center'}}>
          <CustomText style={{}} numberOfLines={1}>
    ...
    <>
      {things && things.length > 0 ? (
        <ScrollView
          ...
          <StatusBar animated={true} backgroundColor={COLOR.primaryColor} />
          ...
          <View style={styles.welcomeMsgContainer}>
            ...
            <CustomText style={{color: COLOR.headerButtonColor}}>
              {t('sentence:greeting')}
            </CustomText>
          </View>
          ...
          {/* ********** (1)  Locations Row ********** */}
          <View style={styles.titleContainer}>
            <CustomText style={styles.rowTitle}>
              {t('navigate:locations')}
            </CustomText>
          </View>
          ...
          <View style={[styles.map, {width: MAP_WIDTH}]}>
            <MapView
              style={{height: MAP_WIDTH, width: '100%', marginTop: 15}}
            ...
            {firstThreeAlerts.map(alert => {
              return (
                <RecentAlertItem
                  ...
                  key={alert.id}
                  id={alert.id}
                  startDatetime={alert.start_datetime}
                  name={alert.thing_name}
