...
const ProfileScreen = (props: any) => {
    ...
    return (
      <TouchableOpacity
        ...
        <View style={[{...styles.pressable}, {...props.style}]}>
          <CustomText style={{color: COLOR.buttonColor}}>
            {props.children}
          </CustomText>
        </View>
      </TouchableOpacity>
  ...
  const onPressAccount = () => {
    return user.admin ? props.navigation.navigate('SwitchAccountScreen') : {};
  };

  return (
    <ScrollView
      contentContainerStyle={{backgroundColor: COLOR.bgColor, flexGrow: 1}}>
      <Card style={styles.card}>
        ...
        <View style={styles.row}>
          ...
          <TouchableOpacity
            ...
            activeOpacity={user.admin ? 0.2 : 1}>
            <CustomText
              style={{
                textAlign: 'center',
                textDecorationLine: user.admin ? 'underline' : 'none',
              }}>
              {account.name}
            </CustomText>
          </TouchableOpacity>
      ...
      <UserButton
        onPress={() => {
          props.navigation.navigate('LanguageSettingScreen');
        }}>
        {t('buttons:languageSetting')}
      </UserButton>
      <UserButton
        onPress={() => {
          props.navigation.navigate('FontSizeSettingScreen');
        }}>
        {t('buttons:fontSizeSetting')}
      </UserButton>
      {/* <UserButton onPress={() => { dispatch(AuthActions.setTutorialSeen(false)); }}>{t('buttons:appTutorial')}</UserButton> */}
      <UserButton
        onPress={() => {
          props.navigation.navigate('AccountMemberListScreen');
        }}>
        {t('buttons:accountMember')}
      </UserButton>
      <UserButton
        onPress={() => {
          props.navigation.navigate('EmergencyContactsScreen');
        }}>
        {t('buttons:emergencyContact')}
      </UserButton>
      <UserButton
        onPress={() => {
          props.navigation.navigate('VideoGuideScreen');
        }}>
        {t('buttons:userGuide')}
      </UserButton>
      <UserButton
      ...
      <View style={{marginTop: 10}}>
        <Text
          style={{textAlign: 'center', fontSize: 13, color: COLOR.shadowColor}}>
