...
const EditDeviceScreen = ({route, navigation}) => {
  ...
  const existingThingsDetails = useSelector((state: RootState) =>
    state.things.things.find(thing => thing.id === thingDetails.id),
  );
  const locations = useSelector(
    (state: RootState) => state.locations.locations,
  ); // for drop down menu
  const thingCurrentLocation = useSelector(
    (state: RootState) => state.locations.locations,
  ).find(location => location.id === thingDetails.locationId);
  const isUserAdmin = useSelector((state: RootState) => state.user.user.admin);
  
  const [name, setName] = useState(existingThingsDetails?.name || '');
  const [isNameEmpty, setIsNameEmpty] = useState(false);
  const [selectedPickerValue, setSelectedPickerValue] = useState<null | number>(
    null,
  );
  ...
  const [selectedMode, setSelectedMode] = useState<string>(thingDetails.mode);

  const [getphoto, setGetPhoto] = useState(
    thingDetails.photo_url ? thingDetails.photo_url : '',
  );
  const [showGetPhoto, setShowGetPhoto] = useState(
    thingDetails.photo_url ? true : false,
  );

  const Label = (props: any) => {
    return (
      <CustomText
        style={{...{fontWeight: 'bold', width: '35%'}, ...props.style}}>
        {props.children}
      </CustomText>
    );
  };
  ...
  function editThingHandler() {
    ...
    dispatch(
      updateDevice(
        ...
        name,
        selectedPickerValue,
        existingThingsDetails?.x_coordinate,
        existingThingsDetails?.y_coordinate,
        showUploadContainer,
        photo,
        'update',
      ),
  ...
  function showSaveAndEditButtonHandler(updateToLocationId: number | null) {
    ...
    const updateToLocation = locations.find(
      location => location.id === updateToLocationId,
    );
    ...
    if (
      thingCurrentLocation?.id != updateToLocationId &&
      updateToLocation?.floorplan
    ) {
          ...
          {isNameEmpty && (
            <View style={styles.notice}>
              <Ionicons
                ...
                style={{width: 26 - 32}}
              />
              <CustomText style={{marginHorizontal: 5, color: '#D96B6F'}}>
                {t('sentence:nameCannotBeEmpty')}
              </CustomText>
            </View>
          )}
              ...
              <View style={[{...styles.row}]}>
                <View style={styles.pickerContainer}>
                  <Picker
                    ...
                    <Picker.Item
                      key=""
                      label={`-- ${t('buttons:selectMode')} --`}
                      value={null}
                      enabled={true}
                    />

                    {THING_MODES.map(mode => {
                      return (
                        <Picker.Item
                          label={t(`common:${mode.key}`)}
                          value={mode.value}
                          key={mode.key}
                        />
                      );
          ...
          {isLoading ? (
            ...
            <View>
              {showSaveAndEditButton && (
                <CustomButton
                  onPress={() => {
                    navigation.navigate('EditCoordinateScreen', {
