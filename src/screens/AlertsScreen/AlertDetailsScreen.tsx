...
const AlertDetailsScreen = ({route, navigation}) => {
  ...
  const alertDetails = useSelector((state: RootState) =>
    state.alerts.allAlerts.find(alert => alert.id === id),
  );
  const thing = useSelector((state: RootState) => state.things.things).find(
    thing => thing.id === alertDetails?.thing_id,
  );
  ...
  ).find(location => location.id === thing?.location_id);
        ...
        <Card style={styles.card}>
          ...
          <View style={styles.detailsContainer}>
            ...
            <Row
              ...
              content={getTranslateType(alertDetails?.alert_type)}
            />
            <Row
              label={t('common:status')}
              content={
                alertDetails?.status === 'Clear'
                  ? t('common:clear')
                  : t('common:set')
              }
            />
            <Row
              ...
              content={formatDateTime(alertDetails?.start_datetime)}
            />
            <Row
              label={t('common:endTime')}
              content={
                alertDetails?.end_datetime
                  ? formatDateTime(alertDetails?.end_datetime)
                  : t('common:null')
              }
            />
            ...
            <Row
              ...
              content={
                alertDetails?.details ? alertDetails?.details : t('common:null')
              }
            />
            ...
            {alertDetails?.photo_url && (
              <View style={styles.row}>
                <View style={styles.photoContainer}>
                  <Image
                    style={styles.photo}
                    source={{uri: alertDetails?.photo_url}}
                  />
                </View>
              </View>
            )}
