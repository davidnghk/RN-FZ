...
import { View, StyleSheet, ScrollView, RefreshControl, Image, TextInput, TouchableOpacity, Switch, Platform, TouchableWithoutFeedback, Keyboard, Text } from 'react-native';
...
const EditAlertScreen = (props: any) => {
    ...
    const thing = useSelector((state: RootState) => state.things.things).find(thing => thing.id === alert.thing_id)
    ...
    const [getphoto, setGetPhoto] = useState(alert.photo_url? alert.photo_url : "")
    const [showGetPhoto, setShowGetPhoto] = useState(alert.photo_url? true : false)
        ...
        {thing &&
            ...
            <Card style={styles.card}>
                ...
                <View style={styles.detailsContainer}>
                    ...
                    <Row label={t('common:alertType')} content={getTranslateType(alert?.alert_type)} />
                    <Row label={t('common:status')} content={alert?.status === 'Clear' ? t('common:clear') : t('common:set')} />
                    <Row label={t('common:startTime')} content={formatDateTime(alert?.start_datetime)} />
                    <Row label={t('common:endTime')} content={alert?.end_datetime ? formatDateTime(alert?.end_datetime) : t('common:null')} />
                    <View style={styles.row}>
                    ...
                    {showGetPhoto && 
                        ...
                        <View style={styles.photoContainer}>
                            <Image
                            source={{ uri: getphoto }}
                            style={{ height: 300, resizeMode: 'contain', backgroundColor: '#d9d9d9' }}
                            />
                        </View>
